...
const TYPE_CFG: Record<string, { label: string; color: string }> = {
  ...
  contractor: { label: "Prestataire", color: "bg-paper-deep text-signal-deep" },
  ...
  return (
    <div className="min-h-screen bg-paper-deep p-6">
      <div className="flex items-center justify-between mb-8">
        ...
        <Button onClick={() => setShowCreate(true)} className="bg-ink text-white gap-2"><Plus className="h-4 w-4" />Nouveau fournisseur</Button>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Fournisseurs actifs", value: stats?.active ?? 0, icon: Building2, color: " to-indigo-600" },
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {[{ v: "all", l: "Tous" }, ...Object.entries(TYPE_CFG).map(([v, c]) => ({ v, l: c.label }))].map(({ v, l }) => {
          ...
          return <button key={v} onClick={() => setFilterType(v)} className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-all ${filterType === v ? "bg-signal-deep text-white border-rule" : "bg-white border-slate-200 hover:border-slate-300"}`}>{l} ({cnt})</button>;
        })}
      </div>
      ...
      : filtered.length === 0 ? (
        <div className="text-center py-16"><Building2 className="h-12 w-12 text-slate-300 mx-auto mb-4" /><h3 className="text-lg font-semibold text-slate-600 mb-1">Aucun fournisseur référencé</h3><Button onClick={() => setShowCreate(true)} className="bg-ink text-white gap-2 mt-4"><Plus className="h-4 w-4" />Ajouter le premier</Button></div>
      ) : (
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      <div className="p-2.5 rounded-xl bg-paper-deep shadow-sm shrink-0"><Building2 className="h-5 w-5 text-signal-deep" /></div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 mt-0.5 flex-wrap text-xs text-slate-500">
                          ...
                          {s.totalSpend ? <span className="text-signal-deep font-medium">{fmtEur(s.totalSpend)}</span> : null}
                          {s.totalOrders ? <span>{s.totalOrders} commandes</span> : null}
                          {s.onTimeDeliveryPct != null && <span>⏱{s.onTimeDeliveryPct}% ponctualité</span>}
                        </div>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><Building2 className="h-5 w-5 text-signal-deep" />Nouveau fournisseur</DialogTitle></DialogHeader>
          <Form {...form}><form onSubmit={form.handleSubmit(d => createMut.mutate(d))} className="space-y-4 mt-2">
            ...
            <DialogFooter><Button type="button" variant="outline" onClick={() => setShowCreate(false)}>Annuler</Button><Button type="submit" disabled={createMut.isPending} className="bg-signal-deep text-white">{createMut.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Plus className="h-4 w-4 mr-2" />}Créer</Button></DialogFooter>
          </form></Form>
        </DialogContent>
      </Dialog>
          ...
          <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
            <DialogHeader>
              <div className="flex items-center gap-3 p-4 rounded-xl bg-paper-deep border border-rule">
                <Building2 className="h-6 w-6 text-signal-deep" />
                ...
                <div className="flex gap-2 flex-wrap">
                  {Object.keys(STATUS_CFG).map(s => (
                    <Button key={s} variant={selected.status === s ? "default" : "outline"} size="sm" className={`text-xs ${selected.status === s ? "bg-signal-deep text-white" : ""}`} onClick={() => updateMut.mutate({ id: selected.id, data: { status: s } })}>{STATUS_CFG[s].label}</Button>
                  ))}
                </div>
              </div>
            </DialogHeader>
              ...
              <TabsContent value="performance" className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    { l: "Dépenses totales", v: fmtEur(selected.totalSpend), c: "text-signal-deep" },
