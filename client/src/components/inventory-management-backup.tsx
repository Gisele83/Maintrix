...
import { Plus, Search, Edit, Trash2, Package, AlertTriangle, TrendingDown, TrendingUp, Download } from "lucide-react";
...
export function InventoryManagement() {
  ...
  const handleExportInventory = () => {
    try {
      ...
      const inventoryContent = generateInventoryReport();
      
      // Create and download the report
      const blob = new Blob([inventoryContent], { type: 'text/html' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `inventaire-pieces-detachees-${new Date().toISOString().split('T')[0]}.html`;
      ...
      toast({
        ...
        description: "L'inventaire des pièces détachées a été exporté avec succès",
      });
    } catch (error) {
      toast({
  ...
  const generateInventoryReport = () => {
    ...
    const lowStockParts = spareParts.filter(p => p.currentStock <= p.minimumStock && p.currentStock > 0);
    const outOfStockParts = spareParts.filter(p => p.currentStock === 0);
    const stockOkParts = spareParts.filter(p => p.currentStock > p.minimumStock);
    ...
    return `
      ...
      <head>
        ...
        <title>Inventaire Pièces Détachées - Smart GMAO DiagFix</title>
        <style>
          ...
          .summary-value { font-size: 28px; font-weight: bold; color: #1F2937; margin-bottom: 5px; }
          .summary-label { font-size: 14px; color: #6B7280; text-transform: uppercase; font-weight: 600; }
          .parts-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
          .parts-table th, .parts-table td { padding: 12px; text-align: left; border-bottom: 1px solid #E5E7EB; }
          .parts-table th { background: #F9FAFB; font-weight: 600; color: #374151; }
          .parts-table tr:hover { background: #F9FAFB; }
          .status-badge { padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: 600; }
          .status-ok { background: #D1FAE5; color: #065F46; }
          .status-low { background: #FEF3C7; color: #92400E; }
          .status-out { background: #FEE2E2; color: #991B1B; }
          .category-badge { padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: 500; background: #EFF6FF; color: #2563EB; }
          .footer { margin-top: 50px; padding-top: 20px; border-top: 1px solid #E5E7EB; text-align: center; color: #6B7280; font-size: 12px; }
          .value-indicator { display: inline-block; background: #10B981; color: white; padding: 8px 16px; border-radius: 20px; font-weight: bold; margin: 10px 0; }
        </style>
      </head>
      <body>
        <div class="header">
          <div class="company-name">Smart GMAO DiagFix</div>
          <div class="report-title">Inventaire des Pièces Détachées</div>
          <div class="report-date">Généré le ${currentDate}</div>
          <div class="value-indicator">
            Valeur totale du stock: ${totalValue.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
          </div>
        </div>

        <div class="section">
          <div class="section-title">Résumé de l'Inventaire</div>
          <div class="summary-grid">
            <div class="summary-card">
              <div class="summary-value">${spareParts.length}</div>
              <div class="summary-label">Pièces au total</div>
            </div>
            <div class="summary-card">
              <div class="summary-value" style="color: #10B981;">${stockOkParts.length}</div>
              <div class="summary-label">Stock OK</div>
            </div>
            <div class="summary-card">
              <div class="summary-value" style="color: #F59E0B;">${lowStockParts.length}</div>
              <div class="summary-label">Stock faible</div>
            </div>
            <div class="summary-card">
              <div class="summary-value" style="color: #EF4444;">${outOfStockParts.length}</div>
              <div class="summary-label">Rupture stock</div>
            </div>
          </div>
        </div>

        <div class="section">
          <div class="section-title">Détail de l'Inventaire</div>
          <table class="parts-table">
            ...
            <tbody>
              ${spareParts.map(part => {
                const stockValue = part.currentStock * part.unitPrice;
                const stockStatus = part.currentStock === 0 ? 'out' : 
                                  part.currentStock <= part.minimumStock ? 'low' : 'ok';
                const statusClass = stockStatus === 'out' ? 'status-out' : 
                                  stockStatus === 'low' ? 'status-low' : 'status-ok';
                const statusText = stockStatus === 'out' ? 'Rupture' : 
                                 stockStatus === 'low' ? 'Stock faible' : 'OK';
                ...
                return `
                  <tr>
                    ...
                    <td><strong>${part.currentStock}</strong></td>
                    <td>${part.minimumStock}</td>
                    <td>${part.unitPrice.toLocaleString('fr-FR', { style: 'currency', currency: part.currency })}</td>
                    <td><strong>${stockValue.toLocaleString('fr-FR', { style: 'currency', currency: part.currency })}</strong></td>
                    <td>${part.location}</td>
        ...
        <div class="section">
          <div class="section-title">⚠️ Pièces à Stock Faible</div>
          <table class="parts-table">
            <thead>
              <tr>
                <th>Numéro</th>
            ...
            <tbody>
              ${lowStockParts.map(part => `
                <tr>
                  <td><strong>${part.partNumber}</strong></td>
                  <td>${part.partName}</td>
                  <td style="color: #F59E0B;"><strong>${part.currentStock}</strong></td>
                  <td>${part.minimumStock}</td>
                  <td style="color: #EF4444;"><strong>${part.minimumStock - part.currentStock + (part.maximumStock ? Math.round((part.maximumStock - part.minimumStock) / 2) : 10)}</strong></td>
        ...
        <div class="section">
          <div class="section-title">🚨 Pièces en Rupture de Stock</div>
          <table class="parts-table">
            <thead>
              <tr>
                <th>Numéro</th>
            ...
            <tbody>
              ${outOfStockParts.map(part => `
                <tr style="background: #FEF2F2;">
                  ...
                  <td>${part.partName}</td>
                  <td>${part.minimumStock}</td>
                  <td style="color: #DC2626;"><strong>Commande URGENTE</strong></td>
                  <td>${part.supplier || 'N/A'}</td>
                </tr>
              `).join('')}
            </tbody>
        ...
        <div class="section">
          ...
          <table class="parts-table">
            ...
            <tbody>
              ${Array.from(new Set(spareParts.map(p => p.category))).map(category => {
                const categoryParts = spareParts.filter(p => p.category === category);
                const categoryValue = categoryParts.reduce((sum, p) => sum + (p.currentStock * p.unitPrice), 0);
                const avgStock = Math.round(categoryParts.reduce((sum, p) => sum + p.currentStock, 0) / categoryParts.length);
                const lowStockCount = categoryParts.filter(p => p.currentStock <= p.minimumStock).length;
                const globalStatus = lowStockCount === 0 ? 'Excellent' : 
                                   lowStockCount <= categoryParts.length * 0.2 ? 'Bon' : 
                                   lowStockCount <= categoryParts.length * 0.5 ? 'Attention' : 'Critique';
                const statusColor = globalStatus === 'Excellent' ? '#10B981' : 
                                   globalStatus === 'Bon' ? '#22C55E' :
                                   globalStatus === 'Attention' ? '#F59E0B' : '#EF4444';
                
                return `
                  <tr>
                    <td><span class="category-badge">${category}</span></td>
                    <td>${categoryParts.length}</td>
                    <td><strong>${categoryValue.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</strong></td>
                    <td>${avgStock}</td>
                    <td style="color: ${statusColor}; font-weight: bold;">${globalStatus}</td>
                  </tr>
                `;
              }).join('')}
            </tbody>
        ...
        <div class="footer">
          ...
          <p>Système de gestion de maintenance assistée par intelligence artificielle</p>
          <p>Données extraites le ${currentDate} - Total: ${spareParts.length} pièces détachées</p>
        </div>
      </body>
      </html>
    `;
...
function SparePartForm({ form, onSubmit, isLoading }: { 
  form: any; 
