...
/**
 * Créateur d'installateur Windows véritable
...
writeFileSync('scripts/windows-installer-real.js', installerScript);

console.log('✅ Script d\'installation créé');

// Créer l'exécutable avec pkg
console.log('📦 Création de l\'exécutable Windows...');

try {
  // Installer pkg si nécessaire
  try {
  ...
  execSync('pkg scripts/windows-installer-real.js --target node18-win-x64 --output scripts/Smart-GMAO-DiagFix-Setup.exe', { 
    stdio: 'inherit',
    cwd: process.cwd()
  });
  
  console.log('✅ Installateur Windows créé avec succès !');
  console.log('📁 Fichier: scripts/Smart-GMAO-DiagFix-Setup.exe');
  
  // Mettre à jour les routes de téléchargement
  console.log('🔄 Mise à jour des routes de téléchargement...');
